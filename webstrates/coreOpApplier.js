...
import {coreMutation} from './coreMutation.js';
import {coreJsonML} from './coreJsonML.js';
import {coreDOM} from './coreDOM.js';
...
// Automerge will generate multiple patches when creating a new JsonML element.
// We'll store the elements while they are constructed in a map, and append them when they are complete.
// The map is from paths in the form of a string, e.g., 2,3,4 to the JsonML object
const elementsInConstruction = new Map();
...
function insertNode(rootElement, path, value) {
	...
	let newElement;
	newElement = typeof value === 'string' ?
	...
	switch (indexType) {
		...
		case jsonml.ATTRIBUTE_INDEX:
			...
			if (attributeName) {
				...
				const newValue = !oldValue || oldValue.length == 0 ? value : oldValue.substring(0, charIndex)
		...
		default:
			...
			let isComment = parentElement.nodeType === document.COMMENT_NODE;
			parentElement = isComment ? parentElement : childElement;
			let oldValue = parentElement.data;
			let newValue = oldValue.substring(0, charIndex)
				+ value + oldValue.substring(charIndex);
			// Generate current text selection range.
			let fakeRange = getSelectionRange(parentElement);
 ...
 * @public
 */
function applyOp(originalOp, rootElement) {
	let charIndex, attributeName;
	const path = structuredClone(originalOp.p);
	if (path.length === 0) {
		return;
	}
	let op = structuredClone(originalOp);
	let lds = []
	if ('d' in op) {
		...
		if (childElement && childElement.nodeName && childElement.nodeName === '#text') {
			...
			for (let i=0; i<op.d; i++) {
				let newOp = {p: structuredClone(op.p), ld: 1};
				lds.push(newOp);
			}
		}
	}
	...
	// attributes.
	// If it is a replacement of a text node, we generate sd and li ops instead and apply those.
	...
	if ('li' in op) {
		//if (!Array.isArray(op.li) && Object.keys(op.li).length === 0) return;
		//if (op.p[op.p.length-1] === 0 && op.li === '') return;
...
coreOpApplierModule.listenForOps = () => {
...
coreOpApplierModule.setRootElement = (rootElement) => {
...
export const coreOpApplier = coreOpApplierModule;
