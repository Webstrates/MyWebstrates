...
//const coreDatabase = require('./coreDatabase'); Moved elementAtPath in here for now....
...
// Snapshot is optional (and only used in the internal recursion).
	if (!path) {
		path = snapshot;
		snapshot = automerge.contentDoc.dom;
	}
...
function attributeMutation(mutation, targetPathNode) {
	...
	const jsonmlAttrs = elementAtPath([...targetPathNodeJsonML, ATTRIBUTE_INDEX]);
...
function characterDataMutation(mutation, targetPathNode) {
	...
	const pathElement = elementAtPath(path);
	...
	ops = coreUtils.patchesToOps(path, oldValue, newValue);
	if (isComment) {
...
function childListMutation(mutation, targetPathNode) {
	...
	Array.from(mutation.removedNodes).forEach(function(removedNode) {
		...
		var jsonmlElement = elementAtPath(path);
...
// Used to avoid adding wids when receiving remote ops when the creation of an element
// and adding the wid has been split into two ops.
coreOpCreatorModule.suppressAddingWids = false;
...
coreOpCreatorModule.addWidToElement = node => {
	...
	if (node.nodeType === document.ELEMENT_NODE && !node.__wid) {
		...
		/*const element = coreDatabase.getDocument(path); // IGNORED FOR NOW.
...
coreOpCreatorModule.ensureExistenceOfWids = targetElement => {
	coreUtils.recursiveForEach(targetElement, node => coreOpCreatorModule.addWidToElement(node));
};
...
coreEvents.addEventListener('DOMNodeInserted', (node, parentElement, local) => {
	...
	if (!local) coreOpCreatorModule.addWidToElement(node);
}, coreEvents.PRIORITY.IMMEDIATE);
...
export const coreOpCreator = coreOpCreatorModule;
