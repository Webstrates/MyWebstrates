...
function ping() {
	if (!window.automerge || window.automerge.handle === undefined) return;
	automerge.handle.broadcast({wa: "ping", body: globalObject.publicObject.clientId, uuid: coreUtils.generateUUID()});
	for (let peer of peers) {
